...
init(void)
{
	...
	DEFiRet;
        ...
        DBGPRINTF("Before sigAct Current UID: %d\n", getuid());
	...
	sigaction(SIGHUP, &sigAct, NULL);
        ...
        DBGPRINTF("Before activate Current UID: %d\n", getuid());

	CHKiRet(rsconf.Activate(ourConf)); // Changed uid from 0 to 101
	DBGPRINTF(" started.\n");

        DBGPRINTF("After activate Current UID: %d\n", getuid());
...
int realMain(int argc, char **argv)
{
	...
	if(myPid != ppid) {
        ...
        DBGPRINTF("Before init() Current UID: %d\n", getuid());

	CHKiRet(init()); // init() changed uid from 0 to 101
	...
	DBGPRINTF("initialization completed, transitioning to regular run mode\n");
        DBGPRINTF("End of initialization Current UID: %d\n", getuid());
